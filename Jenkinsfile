pipeline {
  ...
  stages {
    ...
    stage {
      steps {
        script {
          def images = ["selenium/video:ffmpeg-4.3.1-20221219 ", "selenium/node-edge:4.7.2-20221219", "selenium/node-chrome:4.7.2-20221219", " selenium/node-chrome:4.7.2-20221219", "selenium/node-firefox:4.7.2-20221219", "selenium/node-firefox:4.7.2-20221219", "selenium/hub:4.7.2-20221219"]
          def runningContainers = bat(script: "docker ps --format '{{.Image}}'", returnStdout: true).trim().split("\n")
          def isRunning = images.every {
            img -> runningContainers.contains(img)
          }
          if (isRunning) {
